 ...
 ******************************************************************
 *
 * Module Name : spiceplot.cpp
 *
 * Author/Date : C.B. Lirakis / 10-Nov-05
 *
 * Description : Plot the output from gpsice or gnucap
 *
 *
 ...
 *******************************************************************
 */
SpicePlot::SpicePlot(const TGWindow *p, UInt_t w, UInt_t h, int v) : 
    TGMainFrame( p, w, h,  kVerticalFrame)
{
    // Used to store GUI elements that need to be deleted in the destructor.
    fCleanup = new TList();
    fCleanup->SetOwner();
    Connect("CloseWindow()", "SpicePlot" , this, "CloseWindow()");
 ...
 ******************************************************************
 *
 * Function Name : SpicePlot Destructor
 *
 * Description :
 *
 * Inputs :
 ...
 *******************************************************************
 */
void SpicePlot::AddEmbeddedCanvas(UInt_t w, UInt_t h)
{
    /*
...
void SpicePlot::CreateMenuBar()
{
    ...
    fMenuFile->Connect("Activated(Int_t)", "SpicePlot", this,
		       "HandleMenu(Int_t)");
    fMenuHelp->Connect("Activated(Int_t)", "SpicePlot", this,
		       "HandleMenu(Int_t)");
...
void SpicePlot::CreateToolBar()
{
    ...
    AddFrame(fToolBar, new TGLayoutHints(kLHintsTop | kLHintsExpandX, 0, 0,
					 0, 0));
    fToolBar->Connect("Pressed(Int_t)", "SpicePlot", this, 
		      "HandleToolBar(Int_t)");
}
/**
 ******************************************************************
 ...
 *******************************************************************
 */
void SpicePlot::HandleToolBar(Int_t id)
{
    TGButton *tb;
    TCanvas *c1;
 ...
 *******************************************************************
 */
void SpicePlot::CreateStatusBar()
{
    /*
 ...
 *******************************************************************
 */
void SpicePlot::HandleMenu(Int_t id)
{
    TRootHelpDialog *trh;
    TCanvas *c1;
 ...
 *******************************************************************
 */
void SpicePlot::SetCurrentFileName(const char *File)
{
 ...
 *******************************************************************
 */
void SpicePlot::PlotMe(Int_t Index)
{
    gPad->Clear();
    gPad->Update();
}
 ...
 *******************************************************************
 */
void SpicePlot::ProcessedEvent(Int_t event, Int_t px, Int_t py, 
			     TObject *selected)
{
 ...
 *******************************************************************
 */
void SpicePlot::ZoomAxis(TAxis *a)
{
    Double_t x,y, delta;
    x = a->GetXmin();
    y = a->GetXmax();
 ...
 *******************************************************************
 */
bool SpicePlot::OpenAndParseFile(const char *file)
{
	...
	if(verbose>1) 
	{
	    cout << " Drawing all plots now " << endl;
	}
...
bool SpicePlot::CreateGraphObjects()
{
...
bool SpicePlot::CleanGraphObjects()
{
    gPad->Clear();
