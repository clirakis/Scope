...
ToolBarData_t toolbar_data[] = {
  { "fileopen.xpm"     , "Load SVP file",    kFALSE,     M_FILE_OPEN, NULL },
  { ""                 ,               0,         0,              -1, NULL },
  { "printer.xpm"      ,         "Print",         0,    M_FILE_PRINT, NULL },
  { ""                 ,               0,         0,              -1, NULL },
  { "zoom_selected.xpm",       "Zoom In",     kTRUE, M_ZOOM_SELECTED, NULL },
  { "zoom_minus.xpm"   ,      "Zoom Out",         0,    M_ZOOM_MINUS, NULL },
  { "pointer.xpm"      ,"Select Profile",     kTRUE,  M_ZOOM_POINTER, NULL },
  { NULL               ,            NULL,         0,               0, NULL },
};
#endif
    ...
    TGMainFrame(p, w, h)
{
    ...
    this->SetWindowName("SVP Display");
...
void BuildDialog::CreateToolBar()
{
    ...
    fToolBar->Connect("Pressed(Int_t)", "BuildDialog", this, "HandleToolBar(Int_t)");
}
#endif
/**
 ******************************************************************
...
void BuildDialog::HandleToolBar(Int_t id)
{
    ...
    TListIter next(ToDraw);

    switch (id) 
    {
    case M_FILE_OPEN:
        DoLoad();
        fToolBar->GetButton(M_FILE_OPEN)->SetState(kButtonUp);
	break;
    case M_FILE_PRINT:
	DoPrint(fEmbeddedCanvas->GetCanvas());
	break;
    case M_ZOOM_SELECTED:
        fToolBar->GetButton(M_ZOOM_POINTER)->SetState(kButtonUp);
        // We have to zoom the first SVP which is
        // the only one with an axis
        if ((ts = (TSVP*) next()))
        {
            c1 = fEmbeddedCanvas->GetCanvas();
            c1->SetEditable(kFALSE);
            ts->GetTG()->SetZoom(kTRUE);
        }
	break;
    case M_ZOOM_MINUS:
        fToolBar->GetButton(M_ZOOM_SELECTED)->SetState(kButtonUp);
	UnZoom();
	break;
    case M_ZOOM_POINTER:
        fToolBar->GetButton(M_ZOOM_SELECTED)->SetState(kButtonUp);
